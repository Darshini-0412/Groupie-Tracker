...
// SearchArtistsWithLocations effectue une recherche avancée incluant les lieux et dates
func SearchArtistsWithLocations(artistsEnriched []ArtistEnriched, query string) []SearchSuggestion {
	...
	for _, enriched := range artistsEnriched {
		...
		// 1️⃣ Recherche par nom d'artiste
		...
		// 3️⃣ Recherche par année de création
		...
		// 4️⃣ Recherche par premier album
		...
		// 5️⃣ Recherche par lieux de concert
		if !seen[a.ID] {
		...
		// 6️⃣ Recherche par dates de concert
		if !seen[a.ID] {
...
// SearchArtists effectue la recherche basique (sans lieux/dates)
